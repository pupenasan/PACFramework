...
- [CHCOM]() (CLSID=16\#005x) – комунікаційні канали (загальна інформація)
...
- CHNDI (CLSID=16#006x) – дискретні вхідні комунікаційні канали,
- CHNDO (CLSID=16#007x) – дискретні вихідні комунікаційні канали,
- CHNAI (CLSID=16#008x) – аналогові вхідні комунікаційні канали,
- CHNAO (CLSID=16#009x) – аналогові вихідні комунікаційні канали
...
| CLSID   | UINT | 1 |      | - `CHDI` (CLSID=16#001x) – дискретні вхідні канали, <br />- `CHDO` (CLSID=16#002x) – дискретні вихідні канали; <br />- `CHAI` (CLSID=16#003x) – аналогові вхідні канали;<br />- `CHAO` (CLSID=16#004x) – аналогові вихідні канали. |
| STA | UINT | 2 |  | Може бути набір біт типу `CH_STA` |
| VRAW | BOOL | 2 | 0 | для дискретного сигналу: <br />- для `CHDI` значення з дискретного входу; <br />- для `CHDO` значення на дискретний вихід; <br />- в режимі `FRC=1` може змінюватися із-зовні для `CHDO`; <br />- для CHAI: `RAWINT>0`; <br />- для CHAO: `RAWINT>0` |
| VALB | BOOL | 2 | 1    | значення дискретного сигналу для CM верхнього рівня: <br />- для `CHDI` об'єкт `DIVAR` зчитує його; <br />- для `CHDO` об'єкт `DOVAR` записує в нього; <br />- в режимі `FRC=1` може змінюватися із-зовні для `CHDI`; <br />- для CHAI: `CHCFG.VAL>0`; <br />- для CHAO: `CHCFG.VAL>0` |
...
| VAL | INT | 4 |  | значення: <br />- для `CHAI` значення з аналогового входу для `AIVAR`;<br />- для `CHAO` значення на аналоговий вихід з `AOVAR`;<br />- для `CHDI` = значенню `STA.VAL`;<br />- для `CHDO` = значенню `STA.VAL` |
...
- повинні відслідковуватися та бути реалізовані наступні команди
  ...
  - команди: 
    - 16#0001 - записати `TRUE` (тільки при форсуванні `CHDI/CHDO`)
    - 16#0002 - записати `FALSE` (тільки при форсуванні `CHDI/CHDO`)
    - 16#0003 - `TOOGLE` - змінити на інвертоване значення (тільки при форсуванні `CHDI/CHDO`)
