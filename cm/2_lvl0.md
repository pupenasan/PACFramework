...
|  |  | 0 VRAW | для дискретного сигналу: <br />- для `CHDI` значення з дискретного входу <br />- для `CHDO` значення на дискретний вихід; <br />- в режимі `FRC=1` може змінюватися із-зовні для `CHDO`<br />для CHAI: `RAWINT>0` <br />для CHAO: `RAWINT>0` |
|         |      | 1 VAL    | значення дискретного сигналу для CM верхнього рівня: <br />- для `CHDI` об'єкт `DIVAR` зчитує його; <br />- для `CHDO` об'єкт `DOVAR` записує в нього; <br />- в режимі `FRC=1` може змінюватися із-зовні для `CHDI`<br />для CHAI: `CHCFG.VAL>0` <br />для CHAO: `CHCFG.VAL>0` |
...
| CMD | INT |  | команда:<br />16#1 - записати 1/MAX<br />16#2 - записати 0/MIN<br />16#3 - для дискретних змінних: TOGGLE перемикання 0<->1<br />16#3 - для аналогових змінних: MIDLE виставлення половини з діапазону MIN/MAX <br />16#0100 - прочитати конфігурацію в буфер<br />16#301 - форсувати один/усі об'єкти типу<br />16#302 - дефорсувати один/усі об'єкти типу<br />16#0300 - перемкнути форсування |
