...
| ------- | ---- | ---- | ---- |
| ID      | UINT |      | Унікальний ідентифікатор - номер змінної. Кількість доступних каналів є останнім номером. Нумерація логічна, логіка нумерації вибирається при розробці проекту.ID = 0 зарезервований для помилкових операцій. Реальні канали нумеруються з 1. |
| CLSID   | UINT |      | - `CHDI` (CLSID=16#001x) – дискретні вхідні канали,<br/>- `CHDO` (CLSID=16#002x) – дискретні вихідні канали,<br/>- `CHAI` (CLSID=16#003x) – аналогові вхідні канали,<br/>- `CHAO` (CLSID=16#004x) – аналогові вихідні канали. |
| STA | UINT/INT |  | Може бути набір біт типу `CH_STA` |
|  |  | 0 VRAW | значення дискретного сигналу: <br />- для `CHDI` з дискретного входу <br />- для `CHDO` сигналу на дискретний вихід; <br />- в режимі `FRC=1` може змінюватися із-зовні для `CHDO` |
|         |      | 1 VAL    | значення дискретного сигналу для CM верхнього рівня: <br />- для `CHDI` об'єкт `DIVAR` зчитує його; <br />- для `CHDO` об'єкт `DOVAR` записує в нього; <br />- в режимі `FRC=1` може змінюватися із-зовні для `CHDI` |
|         |      | 2 BAD | = 1 – є якась помилка каналу (`BRK` або `SHRT` або інша) |
|      |      |3 b3|резерв|
|         |      | 4 PNG | = 1 – прийшов PING-запит від власника, обнуляється каналом (PONG-відповідь) |
...
| | | 9 NBD | = 1 – фізично канал не існує; наприклад, для відображення в буфері модуля MODULS для HMI; якщо зручно використовувати неіснуючі канали для кратності адресації |
| | | 10 b10 | резерв |
| | | 11 b11 | резерв |
| | | 12 INBUF | = 1 – змінна `CH`завантажена в буфер `CH_BUF` |
| | | 13 FRC | = 1 – значення форсоване |
| | | 14 SML | = 1 – значення імітується (верхнім рівнем) |
| | | 15 CMDLOAD | =1 - запит на зчитування в буфер (тільки з HMI) |
| CMD | INT |  | команда:<br />16#1 - записати 1/MAX<br />16#2 - записати 0/MIN<br />16#3 - TOGGLE перемикання MIN<->MAX<br />16#0100 - прочитати конфігурацію в буфер<br />16#301 - форсувати один/усі об'єкти типу<br />16#302 - дефорсувати один/усі об'єкти типу<br />16#0300 - перемкнути форсування |
| VAL | INT |  | значення: <br />- для `CHAI` значення з аналогового входу для `AIVAR`<br />- для `CHAO` значення на аналоговий вихід з `AOVAR`<br />- для `CHDI` = значенню `STA.VAL`<br />- для `CHDO` = значенню `STA.VAL` |
| VARID | UINT |  | ID прив'язаної технологічної змінної, 0 - якщо не прив'язана |
### Структура CH\_HMI
| Атрибут | Тип  | Опис                                                      |
| ------- | ---- | --------------------------------------------------------- |
| STA     | INT  | `STA`+`CMD` (біт `X15` – `CMDLOAD` - завантажити в буфер) |
| VAL     | INT  | значення `CH_CFG.VAL`                                     |
...
- IN
  - для вхідних - значення з входів контролера `RAW`
- INOUT
  - для вихідних - значення на виходи контролера `RAW`
  - `CHCFG`
  - `CHHMI`
...
| 8           | змінити значення CHCFG.VAL                                   | значення повинно змінитися на вказане                        | для дискретних, все що більше 0 дорвінює 1                   |
