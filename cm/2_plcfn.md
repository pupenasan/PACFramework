...
## Приклад використання в HMI

Приклад індикації стану ПЛК показаний на рис.1. У даному випадку М – це наявність хоча б одного виконавчого механізму в ручному режимі, F – наявність хоча б одного форсування, S – наявність хоча б одного об'єкта (CM/EM/UNIT) в режимі імітації, W – наявність хоча б однієї тривоги рівня "попередження" (моргає при неквітованій), A – наявність хоча б однієї тривоги рівня "аварія" (моргає при неквітваній). Додатковими бітами статусу для відображення також можуть бути: наявність хоча б одного біта недостовірності, наявність хоча б однієї відключеної змінної (виведеної з обслуговування), "процедура технологічної комірки в роботі/паузі/утримання", "апарат в роботі/паузі/утримання", "CIP в роботі/паузі/утримання".
...
Інший приклад використання PLC_HMI це випадок, коли HMI витягує біти статусів посеред задачі ПЛК. У цьому випадку структура PLC_HMI фіксує ці біти. 
...
|           |                    | 15 CMDACK   | =1 - підтвердження команди, команда пройшла через весь цикл задачі і отримана усіма |
...
| PRM1      | UINT/INT           |             | дискретні параметри                                          |
| PRM2      | UINT/INT           |             | дискретні параметри                                          |
| PLS       | UINT/INT           |             | Може бути набір біт типу PLS                                 |
|           |                    | 0 P100MS    | імпульс на один цикл основної задачі з періодичністю 100 мс  |
|           |                    | 1 P200MS    | 200 мс                                                       |
|           |                    | 2 P500MS    | 500 мс                                                       |
|           |                    | 3 P1S       | 1 с                                                          |
|           |                    | 4 P2S       | 2 с                                                          |
...
|           |                    | 13 NEWHOUR  | =1 (на один цикл основної задачі) – початок години           |
...
| CNTALM    | UINT               |             | кількість активних тривог "аварія"                           |
| CNTWRN    | UINT               |             | кількість активних тривог "попередження"                     |
| CNTBAD    | UINT               |             | кількість активних тривог "недостовірність"                  |
| CNTFRC    | UINT               |             | кількість форсованих об'єктів                                |
| CNTMAN    | UINT               |             | кількість ВМ в ручному режимі                                |
...
- бітові імпульси
- бітові меандри
- імпульси початку години, доби
- скидування бітів статусів та лічильників тривог, статусів
- лічильники роботи ПЛК: загальний і зі старту
- зміни: номер активної зміни, початок зміни
- 
...
- після виклику функції PLCFN при умові, що біт SCN1=TRUE:
  - збільшувати змінну TST_SCN1CLC на 1
  - присвоїти змінній TST_NBCLC := TST_CCLS
...
### Тест астрономічного часу

Пе

### Тест бітових імпульсів
