...
## Способи кастомізації

Щоб не плодити функцій які практично однаково працюють усі вони реалізуються в одній. При цьому є кілька способів кастомізації:

1) виділити об'єкти іншими CLSID, тобто давати їм іншу останню цифру. У цьому випадку функція перевіряє CLSID і в залежності від нього робить чи не робить кастомну функцію. Це робиться для об'єктів, які завжди працюють з цією кастомною функцією  
2) якщо об'єктів мало, прямо в функції можна перевіряти ID, все інше справедливо як для 1-го способу
3) якщо функції виставляються або прибираються при конфігуруванні, варто використати бітові параметри (галочки) в налаштуваннях об'єкту
4) якщо функції виставляються або прибираються в залежності від параметрів інших об'єктів (і тільки так), параметри з варіанту 3 можна змінювати автоматично (автоналаштування)